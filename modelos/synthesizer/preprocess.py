...
def preprocess_dataset(datasets_root: Path, out_dir: Path, n_processes: int, skip_existing: bool, hparams,
    ...
    print("Max input length (text chars): %d" % max(len(m[5]) for m in metadata if len(m) > 5))
    print("Max mel frames length: %d" % max(int(m[4]) for m in metadata if len(m) > 4))

    print("Max audio timesteps length: %d" % max(int(m[3]) for m in metadata if len(m) > 3))


def preprocess_speaker(speaker_dir, out_dir: Path, skip_existing: bool, hparams, no_alignments: bool):
    metadata = []
    for book_dir in chain([speaker_dir], speaker_dir.glob("*")):
        if no_alignments:
            # Gather the utterance audios and texts
            extensions = ["**/*.wav", "**/*.flac", "**/*.mp3"]
            for extension in extensions:
                wav_fpaths = list(book_dir.glob(extension))
